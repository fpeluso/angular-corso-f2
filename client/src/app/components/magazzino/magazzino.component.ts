...
export class MagazzinoComponent implements OnInit {
  ...
  delProduct(i: number): void {
    this.api.deleteProduct(this.prodotti[i]._id['$oid']).subscribe(data => {
      this.updateList();
    })
  }
  ...
  confirm(): void {
    this.api.updateProduct(this.prodotto._id['$oid'], { nome: this.prodotto.nome, descrizione: this.prodotto.descrizione, prezzo: this.prodotto.prezzo, quantita: this.prodotto.quantita }).subscribe(data => {
      this.prodotto = new ProdottoImpl();
